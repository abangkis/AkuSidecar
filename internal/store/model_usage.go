...
func (a *usageAccumulator) add(entry domain.ModelUsageEntry) {
	...
	a.usage.CallerLatencyMS += entry.CallerLatencyMS
...
func aggregateModelUsageEntries(entries []domain.ModelUsageEntry) []domain.ModelUsageEntry {
	...
	for _, entry := range entries {
		...
		current.CallerLatencyMS += entry.CallerLatencyMS
		current.QueueWaitMS += entry.QueueWaitMS
		current.ProviderExecutionMS += entry.ProviderExecutionMS
		current.ResponseTotalMS += entry.ResponseTotalMS
		...
		current.Usage.CallerLatencyMS += entry.Usage.CallerLatencyMS
		current.Usage.QueueWaitMS += entry.Usage.QueueWaitMS
		current.Usage.ProviderExecutionMS += entry.Usage.ProviderExecutionMS
		current.Usage.ResponseTotalMS += entry.Usage.ResponseTotalMS
	}
