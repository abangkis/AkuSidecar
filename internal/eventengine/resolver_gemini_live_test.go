...
func TestLiveGeminiSemanticAcceptanceCanary(t *testing.T) {
	if os.Getenv("AKU_GEMINI_LIVE") != "1" {
		t.Skip("set AKU_GEMINI_LIVE=1 for the Gemini semantic canary")
	}
	...
	cfg, err := config.Load(config.Options{ConfigPath: filepath.Join(root, "config", "sidecar.json"), Provider: "gemini-flash-lite", BridgeExtensionOrigin: "chrome-extension://abcdefghijklmnopabcdefghijklmnop/"})
	if err != nil {
		t.Fatal(err)
	}
	provider, err := reasoning.NewGemini(cfg)
	if err != nil {
		t.Fatal(err)
	}
	...
	if duplicate.Relation != "duplicate_report" || duplicate.TargetAlias == nil || *duplicate.TargetAlias != "candidate_001" || duplicate.Confidence < domain.DefaultSemanticMergeThreshold {
		t.Fatalf("true duplicate was not accepted: %+v", duplicate)
	}
	...
	if usage.Input == nil || usage.Output == nil || duration <= 0 {
		t.Fatalf("missing Gemini telemetry: usage=%+v duration=%s", usage, duration)
	}
}
