...
func TestEmbeddedLibrarySurfaceContract(t *testing.T) {
	for asset, markers := range map[string][]string{
		"web/index.html": {
			...
			"library-load-more",
			"library-topic-knowledge",
			"library-topic-knowledge-results",
			"library-detail",
			"aria-live=\"polite\"",
		},
		"web/app.js": {
			...
			"function buildLibraryTopicKnowledgeCard",
...
func TestHealthAndBootstrapExposeGoBoundary(t *testing.T) {
	...
	response.Body.Close()
	if err != nil || !strings.Contains(string(indexPayload), `rel="icon" type="image/svg+xml" href="/favicon.svg?runtime=release-0.9.0-rc.8"`) {
