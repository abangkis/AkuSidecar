...
func TestHealthAndBootstrapExposeGoBoundary(t *testing.T) {
	...
	for _, marker := range []string{"function reasoningProfileValue", "function syncTimelineSidePanePosition", "reasoningAcquisitionProfile", "reasoningAiDeepProfile", "function recoverInvalidBridgeToken", "invalid_bridge_token", "function runDisabledReason", "Finishing capture cleanup", "BOOTSTRAP_TIMEOUT_MS", "function retryBootstrap", "Retry connection", "session.itemCount"} {
		if !strings.Contains(string(appPayload), marker) {
