...
func TestHealthAndBootstrapExposeGoBoundary(t *testing.T) {
	...
	if bootstrapSettings["timelineBoundaryCueMode"] != "follow" || bootstrapSettings["timelineBoundaryReturnMs"] != float64(350) || bootstrapSettings["semanticEventMergeThreshold"] != .92 || bootstrapSettings["aiDetectionPresentation"] != "drawer" || bootstrapSettings["aiDetectionEnabled"] != true || bootstrapSettings["resurfaceMode"] != "smart" || bootstrapSettings["resurfaceCooldownDays"] != float64(7) {
	...
	for path, markers := range map[string][]string{
		"/":           {"Semantic event engine", "AI Detector", "AI Detection", "ai-detection-enabled", "Resurfaced content", "resurface-mode", "resurface-cooldown-days", "Reasoning processes", "reasoning-processes", "reasoning-executable-path", "detect-reasoning-executable", "ai-detection-presentation", "timeline-side-pane", "semantic-event-shortlist", "semantic-event-merge-threshold", "reset-semantic-event-merge-threshold", "knowledge-retention-days", "knowledge-storage-limit", "timeline-boundary-follow", "timeline-boundary-return-ms", "onboarding-learning-panel", "MEET AKUBROWSER", "WHAT THIS BUILD ADDS", "INSIDE CHECK FOR UPDATES", "First, each source is read and evaluated", "Then, one finite Timeline is composed", "data-onboarding-slide=\"5\"", "onboarding-check-stages"},
