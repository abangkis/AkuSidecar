...
- Deep Detection spends structured inference only on retained posts whose assessment can still change. It skips inadequate text, direct platform/provenance evidence, and active user corrections, and uses its own profile rather than inheriting candidate evaluation. Every model-backed process defaults to Luna `xhigh` and resolves an opaque, user-selectable profile through the active provider's bounded catalog. The current backend is Codex App Server, but the domain contract and Settings renderer are provider-neutral.
