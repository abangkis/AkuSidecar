...
- A source descriptor may declare a bounded follow-up planning policy. Facebook and LinkedIn use the generic `local_frontier` capability: after at least one completed scroll produces zero new candidates and no explicit `has more` signal, AkuSidecar may finish acquisition locally instead of spending a model turn on a no-op follow-up decision. A descriptor may also declare an `initialRoundCandidateTarget`; LinkedIn uses a target of one so a complete, non-deadline-exhausted first round that already found one admissible candidate finishes the current finite check and defers older-feed coverage to a later check. X retains model planning for ambiguous captures, but one mechanically complete shape is local: a complete capture that exhausts its bounded scroll budget while reporting a new candidate, an explicit `has more` signal, and navigable continuation anchors queues exactly one adjacent follow-up round. Incomplete, degraded, deadline-exhausted, contradictory, or unnavigable X evidence still uses the model planner. Candidate evaluation remains model-backed, and continuation-overlap requirements remain unchanged when a follow-up round is requested.
- Reasoning prompts use role-specific transfer objects. Acquisition planning receives only bounded source, candidate-count, capture-quality, scroll-stop, and frontier telemetry; it receives neither captured content nor prior knowledge. Candidate evaluation receives application aliases and bounded evidence text, quote, attachment description, and URL-free media metadata. Native URLs, platform IDs, avatars, presentation state, recovery diagnostics, engagement, links, and attachment/media URLs remain host-owned. Prior knowledge storage is unchanged, while evaluation locally ranks source-scoped history by deterministic actor and lexical overlap, includes a recent fallback, and sends at most 30 entries or 48 KiB. These bounds reduce provider payload without changing the selected reasoning profile or the host-side evidence binding contract.
...
- `show_all` bypasses event retrieval and resolution. User event corrections are local, persistent, and undoable. The correction contract records event membership separately from novelty, allowing `not the same event`, `same event Â· repeated report`, and `same event Â· meaningful update` without conflating those decisions.
...
- Drawer is the preview default and never abruptly removes a post already seen inline. Inline remains selectable. Hide requires exact typed confirmation and accepts only direct evidence, Deep-confirmed strong signals, or an explicit user AI verdictâ€”not preliminary inference.
...
SQLite schema version 10 contains only active tables for metadata, source definitions, settings, sessions/runs, Bridge commands/observations, reasoning telemetry, durable assessments for every evaluated candidate, Timeline and append-only selection corrections, detector-owned AI assessment history and asynchronous jobs, canonical append-only personal AI feedback, item-scoped media recaptures and evidence overrides (including passive-enrichment provenance), calibration, preference/model state, source-scoped knowledge, native-content continuity and identity promotion, semantic event reports, bounded event deltas, membership constraints, novelty constraints, corrections, resolver/trigger telemetry, and an aggregate-only semantic signal receipt captured at shortlist construction. The receipt stores no candidate text, URLs, names, claims, event keys, or prompt fragments; old rows remain receipt-unavailable. Source-bearing rows reference the application registry through `source_definitions`; adding a source no longer requires editing every table constraint. Mutable bounded payloads use JSON; lifecycle, integrity, and ordering fields remain typed columns.
